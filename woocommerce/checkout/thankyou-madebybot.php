...
// Ensure we have an order object (Woo usually passes $order in).
...
/**
 * Helper: render up to 3 item thumbnails with quantity badges.
 *
 * @param WC_Order $order
 */
if ( ! function_exists( 'ab_render_order_thumbs' ) ) {
	function ab_render_order_thumbs( $order ) {
		...
		foreach ( $order->get_items() as $item_id => $item ) {
			...
			$thumb = $product->get_image( 'woocommerce_thumbnail', array( 'class' => 'w-full h-full object-cover' ) );
			$qty   = $item->get_quantity();

			echo '<div class="ab-thumb relative w-[80px] h-[96px] overflow-hidden bg-[#F3F4F6] flex items-center justify-center">';
			echo $thumb ? $thumb : '<span class="block w-[40px] h-[40px] bg-[#e5e7eb]"></span>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			echo '<span class="ab-badge absolute -top-2 -right-2">' . esc_html( $qty ) . '</span>';
			echo '</div>';

			$i++;
			if ( $i >= 3 ) {
				break; // Show up to 3 like the mock.
			}
		}
	}
}

// If we have an order, fire the standard WooCommerce hook BEFORE thankyou content.
// Plugins (including some tracking integrations) may hook here.
if ( $order ) {
...
<section class="ab-thankyou theme-grid relative py-10 md:pb-16 md:pt-0">
	<div class="col-span-2 md:col-span-6 xl:col-span-8 col-start-1 md:col-start-1 xl:col-start-3">
		...
		<div class="ab-steps max-w-4xl mx-auto mb-20 hidden xl:flex items-center justify-between">
			<div class="step step-active flex items-center gap-3 pb-6 border-b-2 border-[#38CB89] w-full md:w-[30%] mr-[3.333333%]">
				...
				<span class="font-barlow text-[16px] leading-[26px] font-semibold text-[#38CB89]">
					<?php esc_html_e( 'Warenkorb', 'aleandbread' ); ?>
				</span>
			</div>
			<div class="step step-active flex items-center gap-3 pb-6 border-b-2 border-[#38CB89] w-full md:w-[30%] mr-[3.333333%]">
				<span class="step-dot-done">✓</span>
				<span class="font-barlow text-[16px] leading-[26px] font-semibold text-[#38CB89]">
					<?php esc_html_e( 'Bestelldetails', 'aleandbread' ); ?>
				</span>
			</div>
			<div class="step step-active flex items-center gap-3 pb-6 w-full border-b-2 border-dark md:w-[30%] mr-[3.333333%]">
				...
				<span class="font-barlow text-[16px] leading-[26px] font-semibold text-dark">
					<?php esc_html_e( 'Bestellung abgeschlossen', 'aleandbread' ); ?>
				</span>
			</div>
		</div>
		...
		<?php if ( $order ) : ?>
				...
				<div class="max-w-3xl mx-auto">
					<div class="ab-card bg-white shadow-[0_1px_2px_rgba(0,0,0,.04),0_12px_40px_rgba(0,0,0,.06)] px-12 py-10 md:px-24 md:py-20 text-center">
						<div class="ty-msg-wrapper mb-7 flex flex-row items-center justify-center">
							<p class="text-[#6C7275] font-barlow text-[28px] font-medium tracking-[-0.6px] mr-1">
								<?php esc_html_e( 'Danke!', 'aleandbread' ); ?>
							</p>
						</div>
						<p class="text-[22px] md:text-[40px] leading-tight font-medium text-dark mb-12 max-w-[492px] mx-auto">
							<?php esc_html_e( 'Ihre Bestellung ist eingegangen', 'aleandbread' ); ?>
						</p>

						<!-- Thumbnails row -->
						<div class="flex items-center justify-center gap-14 mb-12">
							<?php ab_render_order_thumbs( $order ); ?>
						</div>
						...
						// Optional: show Email similar to core template.
						if (
							is_user_logged_in()
						...
						<dl class="ab-meta mx-auto w-full max-w-xl grid grid-cols-2 gap-x-8 gap-y-3 items-start text-[14px] md:text-base leading-6">
							<?php foreach ( $rows as $r ) : ?>
								<dt class="text-[#6C7275] whitespace-nowrap">
									<?php echo esc_html( $r['label'] ); ?>
								</dt>
								<dd class="font-semibold text-dark break-words">
									<?php
									// Some values may already be escaped/formatted (e.g. totals),
									// so we echo them directly.
									echo is_string( $r['value'] ) ? $r['value'] : wp_kses_post( $r['value'] ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
									?>
								</dd>
							<?php endforeach; ?>
						</dl>
			...
			<?php
			/**
			 * IMPORTANT FOR GTM/GA:
			 * These are the standard WooCommerce thankyou hooks.
			 * Most GA / GTM / tracking plugins hook into one or both of these.
			 */
		...
		<?php else : ?>
			...
			<!-- Fallback when no order is found -->
			<div class="max-w-3xl mx-auto">
				<div class="ab-card bg-white shadow-[0_1px_2px_rgba(0,0,0,.04),0_12px_40px_rgba(0,0,0,.06)] px-12 py-10 md:px-24 md:py-20 text-center">
					<p class="text-[#6C7275]">
						<?php esc_html_e( 'Vielen Dank. Ihre Bestellung ist eingegangen.', 'aleandbread' ); ?>
					</p>
				</div>
			</div>

			<?php
			// For maximum parity with core you could also include:
