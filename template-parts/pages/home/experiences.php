<section id="experiences-overview" class="experiences-overview bg-dark pb-20 md:pb-48 xl:pb-56">
	<div class="theme-container">
		...
		if ( $shop_exp_cats ) :
			...
			<ul class="shop-categories theme-grid mb-14 xl:mb-16">
				...
				foreach ( $shop_exp_cats as $exp_cat ) :
					...
					?>
					<li class="card-category card-category--experiences col-span-2 md:col-span-3 xl:col-span-4 overflow-hidden" style="background-image:url(<?php echo esc_url( $img_url ); ?>); background-size: cover; background-position: center; background-repeat: no-repeat;">
		...
		<div class="theme-grid">
			<div class="col-span-2 md:col-span-6 xl:col-span-12 flex justify-center items-center">
				...
				if ( $btn ) :
					?>
					<a class="btn btn-primary invisible hidden md:visible md:block" href="<?php echo esc_url( $btn ); ?>" target="_self"><?php echo esc_html( ! empty( trim( (string) $btn_text ) ) ? $btn_text : __( 'Jetzt buchen', 'aleandbread' ) ); ?></a>
					<?php
				endif;
				?>
			</div>
